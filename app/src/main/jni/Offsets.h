...
bool isAutoFindDump = false, isDumpLib = false, isDumpLibDone = false, isLibDumped = false, isFastDump = false, isRebuiltLibDump = false, isStringsDump = false, isStringDumped = false, isObjectsDump = false, isObjectsDumped = false, isSDKUDump = false, isSDKUDumped = false, isSDKWDump = false, isSDKWDumped = false, isActorsDump = false, isActorsDumped = false, isBoneDump = false, isBoneDumped = false;

namespace Offsets {
    uintptr_t PointerSize = sizeof(void*);

    bool isUE423 = true, isUE5 = false, isXorDecrypt = false;
    uint8_t XorKey = 0; // 0 = default string length XOR, otherwise use this key

    // Anti-Cheat evasion: toggle stealth read mode (uses /proc/pid/mem instead of vm_readv)
    bool useStealthRead = false;

    // EngineOffsets
    uintptr_t GWorld = 0x0;
    uintptr_t GNames = 0x0;
    uintptr_t GUObjectArray = 0x0; // whatever found in IDA do a (minus) - FUObjectArrayToTUObjectArray (0x10) to get the value or set FUObjectArrayToTUObjectArray to 0

    // TNameEntry (kept for fallback compatibility)
    uintptr_t TNameEntryToNameString = 0x10;

    // FNamePool
    uintptr_t GNamesToFNamePool = 0x0;
    uintptr_t FNamePoolToBlocks = 0x10;
    uintptr_t FNameStride = 0x2; // 0x2 for UE4.23+, 0x4 for UE5.0+
    ...
    */
    ...
    void ExampleGame() {
        Memory.GameName = "Test Game";
        Memory.TargetProcess = "libUE4.so";

        isUE423 = true; // UE4.23+ mode (NOT UE5 — FNameStride=0x2, not 0x4)
